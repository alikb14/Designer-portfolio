...
- Play cards now use a short staggered `play-card-enter` reveal from below on initial load, starting at 700/800/900ms to synchronize with the corresponding Play typewriter starts; reduced-motion disables the entrance animation while preserving the final visible state.
